...
> **v0.1 變更摘要**：initial draft。新 remote 已開好 `https://github.com/ddwolfer/Multi-knowledgeGraph`，採 `.git` reset 全新 history。
...
6. 第一次 commit：`feat: initial fork from knowledgeGraph + multi-DB support`
...
頂部加 attribution（user 指定的「從 XX github 更改」）：

```markdown
> Fork of [ddwolfer/knowledgeGraph](https://github.com/ddwolfer/knowledgeGraph)
> （original upstream: [ChenLiangChong/knowledgeGraph](https://github.com/ChenLiangChong/knowledgeGraph)）
...
- [ ] **Q1**：Attribution 要不要也列 ChenLiangChong（原始上游）？建議「兩個都列」——直接 fork 來源 + 原始 upstream。或只列直接來源 `ddwolfer/knowledgeGraph` 也行。
- [ ] **Q2**：`search-enforcer.js` 不開 DB，只用 flag 檔案做 enforcement，要不要也支援「per-DB 獨立 enforcer 狀態」？目前是一個 global 狀態檔，多 DB 場景下會混。建議 v1 不動（YAGNI），v2 再看是否需要。
- [ ] **Q3**：第一個 commit 訊息要不要包含「inspired by / forked from」字眼？還是純粹的 `initial commit`？
