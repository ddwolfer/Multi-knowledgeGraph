...
> **v0.3 變更摘要**:全數決策定案、`status: approved`。執行採方案 B（先做第一輪通用引擎，第二輪等開課前再做）;Whisper 備援鎖 `faster-whisper`。

## 目標

**先講定位（回應「不只學習會用到」）**:Multi-KG 是**通用 KG 引擎**，不是學習專用工具。它服務知識的兩個面向——

- **獲取面**:怎麼「養出」一個領域專家庫（本次的 study scaffold:上課 → 入庫 → 養成 `system-design.db`）。
- **應用面**:怎麼把養好的專家「招募」進任何專案（web、遊戲、後端…）:用既有的 `kg-init` 把 KG 接進該專案，再用 `merge-db.js` 把一個或多個 master 合進該專案的工作大腦。

所以 **web/game 等用途早已被這份計畫服務到**:它們就是下方架構圖裡「未來工作專案 / team.db」那一格，靠第一輪的去耦 + merge 達成。study 只是 `templates/` 底下的**第一個** scaffold，被刻意隔離、絕不汙染引擎；日後要加 `templates/web/`、`templates/game/` 等其他 archetype 也不衝突。
...
| 項目 | 決策 | 來源 |
|------|------|------|
| Multi-KG 定位 | **通用 KG 引擎**;study 是第一個 scaffold 而非唯一用途。web/game 等用 `kg-init` 接入 + `merge-db` 招募 master | 使用者澄清「不只學習會用到」 |
| 執行順序 | **方案 B**:先做第一輪（通用引擎基礎），第二輪 scaffold 等開課前再做 | 使用者選 B |
...
- [x] **Q1**:Gemini 模型 → `ask_video`=Flash、`digest_lesson`=Pro。
- [x] **Q2**:generator → 新增獨立 `init-study-project.js`（職責清楚）。
- [x] **Q3**:Whisper → `faster-whisper`（本機免費、約 4× 快、品質與 `openai-whisper` 相同——同一份模型權重;僅作 Gemini 的備援）。
- [x] **Q4**:merge 去重 → 先只做 UUID 級;同名不同義不自動合併（日後再議 `--dedup-similar`）。
- [x] **Q5**:TDD → `merge-db.js` 走 TDD（用 Node 內建 `node:test`，不加新依賴）;純文字去耦不走 TDD。
- [x] **執行順序** → 方案 B:先做第一輪，第二輪等開課前再做。
...
- ❌ **其他 archetype 樣板（`templates/web/`、`templates/game/` 等）** — 結構已支援，本次不做（YAGNI;web/game 先用既有 `kg-init` 接入 + `merge-db` 招募 master 即足夠）
...
**v0.3（2026-05-31）** — 決策全定案、`approved`;執行採方案 B、Whisper 鎖 `faster-whisper`、TDD 用 `node:test`
**v0.2（2026-05-31）** — 釘死「通用引擎」定位:新增定位段落、決策列、`templates/<archetype>` 出範圍說明（回應 web/game 也要用）
