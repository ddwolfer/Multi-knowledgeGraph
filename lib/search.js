...
import { ftsQuery as buildFtsQuery } from './tokenize.js';
...
export async function hybridSearch(query, { mode = 'hybrid', limit = 10 } = {}) {
  ...
  if (mode !== 'vector') {
    ...
    // Shared with auto-recall. Splitting on whitespace alone drops Chinese
    // entirely, because it has none -- see lib/tokenize.js.
    const ftsQuery = buildFtsQuery(sanitized, 10);
